...
import mocDataList from "../../store/mocListsData";
...
const LeftSidebar = ({ sidebarToggle, sidebarWidth, setSidebarWidth }) => {
  ...
  const handleItemClick = (index) => {
    setComponentAllState({
      ...componentAllState,
      selectedNavMenuItem: UserProjectList.length + 6 + index,
    });
  };
  ...
  const handleAddNewProject = () => {
    const tempTextNewProject = {
      ...
      isFavorite: true,
      taskLists: [...mocDataList], // ... testing..
    };
    dispatch(addNewProject(tempTextNewProject));
    handleActiveLinkState(4);
  };

  const ShowCurrentBoard = (board, index) => {
    setComponentAllState({ ...componentAllState, selectedProject: index });
  ...
  return (
    <aside
      className={`bg-white py-2 h-full border-r border-gray-400 absolute top-0 md:left-0 md:relative ${
    ...
    >
      ...
      <nav className="relative w-full bg-inherit grid grid-cols-1 grid-rows-[1fr_110px] h-[calc(100%-65px)] pr-2">
        <div
          className={`absolute top-1 right-2 bg-inherit p-2 rounded-full cursor-pointer ${
            !sidebarWidth && "hidden"
          }`}
        ...
        <div className="overflow-y-auto overflow-x-hidden h-full bg-inherit">
          <div className="py-2 bg-inherit">
            ...
            <ul
              ...
              role="list"
            >
              <li onClick={() => handleActiveLinkState(1)}>
                <Link
                  to={"./"}
                  className={`flex items-center px-3 py-2 text-md rounded-e-3xl cursor-pointer hover:bg-slate-200 hover:text-slate-800 hover:font-semibold transition-all  duration-200 ${
                    sidebarWidth &&
                    componentAllState.selectedNavMenuItem == 1 &&
                    "pl-6"
                  } ${
                    componentAllState.selectedNavMenuItem == 1 &&
                    !sidebarWidth &&
                ...
                >
                  ...
                  />
                  {sidebarWidth && "Dashboard"}
                </Link>
              </li>
              <li onClick={() => handleActiveLinkState(2)}>
                <Link
                  to={"./notification"}
                  className={`flex items-center px-3 py-2 text-md cursor-pointer hover:pl-6 hover:bg-slate-200 hover:text-slate-800 hover:font-semibold transition-all duration-200 rounded-e-3xl ${
                    sidebarWidth &&
                    componentAllState.selectedNavMenuItem == 2 &&
                    "pl-6"
                  } ${
                    componentAllState.selectedNavMenuItem == 2
                      ? "text-slate-800 bg-gray-200 font-semibold *:scale-110 border border-neutral-400"
                ...
                >
                  <IoNotificationsOutline
                    className={`mr-3 text-xl -ml-0.5 ${
                      componentAllState.selectedNavMenuItem == 2
                        ? "custom-Stroke-width-dark"
                        : "custom-Stroke-width-light"
                    }`}
                  />
                  {sidebarWidth && "Notification"}
                </Link>
              </li>
              <li className="bg-inherit">
                <div
                  className={`bg-inherit relative z-[1] flex items-center gap-2 py-1 text-md cursor-pointer hover:bg-slate-200 hover:text-slate-800 hover:font-semibold transition-all duration-200 group rounded-e-3xl ${
                    componentAllState.selectedNavMenuItem == 3
                    ...
                    !sidebarWidth ? "hover:pl-6 pl-5 pr-0 py-2" : "hover:pl-6"
                  }`}
                  onClick={() =>
                    handleActiveLinkState(3, {
                      showAllProjectList: !componentAllState.showAllProjectList,
                    })
                  }
                >
                  <IoBriefcaseOutline
                    ...
                    >
                      ...
                      <IoArrowDownCircleOutline
                        className={`group-hover:text-gray-900 inline-block w-8 h-8 p-[4px] text-gray-500 ${
                          componentAllState.showAllProjectList && "rotate-180"
                  ...
                  (UserProjectList.length < 10 ? (
                    <div
                      className={`w-fit py-2 mt-1 cursor-pointer rounded-e-3xl hover:bg-gray-200 hover:pl-6 hover:text-slate-900 hover:font-semibold transition-all duration-200 relative group overflow-hidden ${
                        componentAllState.selectedNavMenuItem == 4
                        ...
                        <li
                          className={`ml-2 relative after:absolute after:left-[-1px] after:z-10 after:top-1/2 after:bg-slate-300 hover:after:bg-slate-600 after:h-[6px] after:w-[6px] after:translate-y-[-50%] after:translate-x-[-50%] after:rounded-full ${
                            componentAllState.selectedNavMenuItem ==
                              index + 4 && "after:bg-slate-600"
                          }`}
                          onClick={() => {
                            ShowCurrentBoard(project, index);
                            handleActiveLinkState(index + 4);
                          }}
                          key={project.id}
                        >
                          <Link
                            ...
                            className={`cursor-pointer relative px-3 py-2 text-sm font-poppins rounded-e-3xl group flex items-center gap-2 justify-between transition-all hover:pl-6 hover:text-slate-800 hover:font-semibold ${
                              componentAllState.selectedNavMenuItem == index + 4
                          ...
                          >
                            ...
                            <span className="" onClick={() => setFavProject()}>
                              {project?.isFavorite && (
              ...
              <li
                onClick={() =>
                  handleActiveLinkState(UserProjectList.length + 5)
                }
              >
                <Link
                  to={"./timeline"}
                  className={`flex items-center px-3 py-2 text-md cursor-pointer hover:bg-slate-200 hover:font-semibold hover:text-slate-800 rounded-e-3xl transition-all duration-200 ${
                    componentAllState.selectedNavMenuItem ==
                    UserProjectList.length + 5
                      ? "text-slate-800 bg-gray-200 font-semibold *:scale-110 border border-neutral-400"
                      : "text-slate-600 font-medium pl-3 *:scale-100 border border-transparent"
                  }  ${
                    sidebarWidth &&
                ...
                >
                  <MdOutlineViewTimeline className={`mr-3 text-xl`} />
                  {sidebarWidth && "Timeline"}
                </Link>
              </li>
            </ul>
          </div>
          {sidebarWidth && (
            <div className="border-t-[1px] border-slate-300 pt-2">
              <button
                ...
                onClick={() =>
                  handleActiveLinkState(componentAllState.selectedNavMenuItem, {
                    showFavProjectList: !componentAllState.showFavProjectList,
                  })
                }
              >
                My Favorite
              ...
              {componentAllState.showFavProjectList && (
                <ul className="py-3 flex flex-col gap-2 pr-2">
                  {UserProjectList.filter((project) => project.isFavorite).map(
                    (project, index) => (
                      <li
                        key={project.id}
                      ...
                      >
                        <Link
                          ...
                          className={`flex items-center gap-2 px-2 py-2 text-sm  font-medium hover:text-gray-900 text-gray-500 font-poppins cursor-pointer relative hover:bg-gray-200 hover:font-semibold hover:pl-6 group rounded-e-3xl transition-all duration-200 capitalize ${
                            componentAllState.selectedNavMenuItem ==
                            UserProjectList.length + 6 + index
        ...
        <ul className={`py-3 flex flex-col gap-2`}>
          <li
            className={`flex items-center px-3 py-2 text-md rounded-e-3xl font-medium cursor-pointer hover:pl-6 hover:bg-slate-200 hover:text-slate-800 hover:font-semibold transition-all duration-200 text-md text-gray-800 ${
              componentAllState.selectedNavMenuItem ==
              UserProjectList.length +
                UserProjectList.filter((list) => list.isFavorite).length +
                6
                ? "text-slate-800 bg-gray-200 font-semibold *:scale-110 border border-neutral-400"
                : "text-slate-600 font-medium pl-3 *:scale-100 border border-transparent"
            } ${
          ...
          <li
            className={`flex items-center px-3 py-2 text-sm rounded-e-3xl hover:font-semibold cursor-pointer hover:bg-slate-200 hover:text-slate-800 hover:pl-6 transition-all duration-200 text-md ${
              componentAllState.selectedNavMenuItem ==
              UserProjectList.length +
                UserProjectList.filter((list) => list.isFavorite).length +
                7
              ...
              componentAllState.selectedNavMenuItem ==
                UserProjectList.length +
                  ...
                  7 &&
              "pl-2"
            } ${!sidebarWidth ? "hover:pl-6 pl-4 pr-0" : "hover:pl-6"}`}
            onClick={() =>
              handleActiveLinkState(
                UserProjectList.length +
                  UserProjectList.filter((list) => list.isFavorite).length +
          ...
          >
            ...
            {sidebarWidth && currentUser.username}
          </li>
        </ul>
      </nav>
    </aside>
